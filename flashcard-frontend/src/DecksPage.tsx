...
const DecksPage: React.FC = () => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      await apiRequest(
        '/decks',
        { method: 'POST', body: JSON.stringify({ name: newDeck }) },
        token!
      );
      setNewDeck('');
      fetchDecks();
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await apiRequest(
        `/decks/${editingId}`,
        { method: 'PUT', body: JSON.stringify({ name: editingName }) },
        token!
      );
      setEditingId(null);
      setEditingName('');
  ...
  return (
    <div>
      ...
      <div className="container">
        ...
        <div className="card">
          ...
          <form onSubmit={handleCreate} style={{ display: 'flex', gap: '12px' }}>
            <input
              ...
              style={{ flex: 1 }}
            />
            <button type="submit" className="btn">Create Deck</button>
          </form>
        </div>
        ...
        {/* Loading Spin */}
        {loading && <div className="card">Loading decks...</div>}
                ...
                {editingId === deck.id ? (
                  ...
                  <>
                    ...
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                      <button onClick={() => { setEditingId(deck.id); setEditingName(deck.name); }} className="btn btn-secondary">
                        Edit
                      </button>
